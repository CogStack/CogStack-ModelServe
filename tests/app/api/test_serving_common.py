...
ANOTHER_TRAINER_EXPORT_PATH = os.path.join(
    os.path.dirname(__file__), "..", "..", "resources", "fixture", "another_trainer_export.json"
)
TRAINER_EXPORT_MULTI_PROJS_PATH = os.path.join(
    os.path.dirname(__file__), "..", "..", "resources", "fixture", "trainer_export_multi_projs.json"
)
MULTI_TEXTS_FILE_PATH = os.path.join(
    os.path.dirname(__file__), "..", "..", "resources", "fixture", "sample_texts.json"
)
PUBLIC_KEY_PEM_PATH = os.path.join(
    os.path.dirname(__file__), "..", "..", "resources", "fixture", "public_key.pem"
)
...
def test_redact_with_white_list(model_service, client):
    ...
    response = client.post(url, data="Spinal stenosis", headers={"Content-Type": "text/plain"})
...
def test_redact_with_encryption(model_service, client):
    ...
    response = client.post(
        "/redact_with_encryption", json=body, headers={"Content-Type": "application/json"}
    )
...
def test_train_supervised(model_service, client):
    ...
    with open(TRAINER_EXPORT_PATH, "rb") as f:
        response = client.post(
            f"/train_supervised?tracking_id={TRACKING_ID}", files=[("trainer_export", f)]
        )
...
def test_train_unsupervised(model_service, client):
    ...
    with tempfile.TemporaryFile("r+b") as f:
        f.write(str.encode('["Spinal stenosis"]'))
        response = client.post(
            f"/train_unsupervised?tracking_id={TRACKING_ID}", files=[("training_data", f)]
        )
...
def test_train_unsupervised_with_hf_hub_dataset(model_service, client):
    ...
    # test with provided tracking ID
    response = client.post(
        f"/train_unsupervised_with_hf_hub_dataset?hf_dataset_repo_id=imdb&tracking_id={TRACKING_ID}"
    )
...
def test_train_metacat(model_service, client):
    ...
    with open(TRAINER_EXPORT_PATH, "rb") as f:
        response = client.post(
            f"/train_metacat?tracking_id={TRACKING_ID}", files=[("trainer_export", f)]
        )
...
def test_evaluate_with_trainer_export(client):
    ...
    with open(TRAINER_EXPORT_PATH, "rb") as f:
        response = client.post(
            f"/evaluate?tracking_id={TRACKING_ID}", files=[("trainer_export", f)]
        )
...
def test_sanity_check_with_trainer_export(client):
    ...
    with open(TRAINER_EXPORT_PATH, "rb") as f:
        response = client.post(
            f"/sanity-check?tracking_id={TRACKING_ID}", files=[("trainer_export", f)]
        )
...
def test_inter_annotator_agreement_scores_per_concept(client):
    ...
    assert (
        response.text.split("\n")[0]
        == "concept,iaa_percentage,cohens_kappa,iaa_percentage_meta,cohens_kappa_meta"
    )
...
@pytest.mark.parametrize(
    "pid_a,pid_b,error_message",
    [(0, 2, "Cannot find the project with ID: 0"), (1, 3, "Cannot find the project with ID: 3")],
)
...
def test_inter_annotator_agreement_scores_per_doc(client):
    ...
    assert (
        response.text.split("\n")[0]
        == "doc_id,iaa_percentage,cohens_kappa,iaa_percentage_meta,cohens_kappa_meta"
    )
...
def test_inter_annotator_agreement_scores_per_span(client):
    ...
    assert response.headers["Content-Type"] == "text/csv; charset=utf-8"
    assert response.text.split("\n")[0] == (
        "doc_id,span_start,span_end,iaa_percentage,cohens_kappa,"
        "iaa_percentage_meta,cohens_kappa_meta"
    )
...
def test_get_annotation_stats(client):
    ...
    assert (
        response.text.split("\n")[0]
        == "concept,anno_count,anno_unique_counts,anno_ignorance_counts"
    )
...
def test_extract_entities_from_text_list_file_as_json_file(model_service, client):
    ...
    assert (
        ...
        == [
            {
                "text": (
                    "Description: Intracerebral hemorrhage (very acute clinical changes occurred"
                    " immediately).\nCC: Left hand numbness on presentation; then developed"
                    " lethargy later that day.\nHX: On the day of presentation, this 72 y/o RHM"
                    " suddenly developed generalized weakness and lightheadedness, and could not"
                    " rise from a chair. Four hours later he experienced sudden left hand numbness"
                    " lasting two hours. There were no other associated symptoms except for the"
                    " generalized weakness and lightheadedness. He denied vertigo.\nHe had been"
                    " experiencing falling spells without associated LOC up to several times a"
                    " month for the past year.\nMEDS: procardia SR, Lasix, Ecotrin, KCL, Digoxin,"
                    " Colace, Coumadin.\nPMH: 1)8/92 evaluation for presyncope (Echocardiogram"
                    " showed: AV fibrosis/calcification, AV stenosis/insufficiency, MV stenosis"
                    " with annular calcification and regurgitation, moderate TR, Decreased LV"
                    " systolic function, severe LAE. MRI brain: focal areas of increased T2 signal"
                    " in the left cerebellum and in the brainstem probably representing"
                    " microvascular ischemic disease. IVG (MUGA scan)revealed: global hypokinesis"
                    " of the LV and biventricular dysfunction, RV ejection Fx 45% and LV ejection"
                    " Fx 39%. He was subsequently placed on coumadin severe valvular heart"
                    " disease), 2)HTN, 3)Rheumatic fever and heart disease, 4)COPD, 5)ETOH abuse,"
                    " 6)colonic polyps, 7)CAD, 8)CHF, 9)Appendectomy, 10)Junctional tachycardia."
                ),
                "annotations": [
                    {
                        "label_name": "Spinal stenosis",
    ...
    with open(MULTI_TEXTS_FILE_PATH, "rb") as f:
        response = client.post(
            f"/process_bulk_file?tracking_id={TRACKING_ID}", files=[("multi_text_file", f)]
        )
