...
from trainers.medcat_deid_trainer import (
    LabelCountCallback,
    MedcatDeIdentificationSupervisedTrainer,
    MetricsCallback,
)
...
def test_medcat_deid_supervised_run(mlflow_fixture):
    with open(os.path.join(data_dir, "trainer_export.json"), "r") as data_file:
        MedcatDeIdentificationSupervisedTrainer.run(
            deid_trainer, {"nepochs": 1, "print_stats": 1}, data_file, 1, "run_id"
        )
