...
def get_train_eval_metrics(train_eval_id: str, training_api: TrainingApi) -> dict:
    """
    ...
    """
    try:
        api_response = training_api.train_eval_metrics(train_eval_id)
        return api_response[0]  # type: ignore
    except ApiException as e:
        logger.error(f"API Exception when calling train_eval_metrics: {e}")
        return {
            "status": "error",
            "reason": e.reason,
        }
    except TimeoutError:
        logger.error("Request timed out when calling train_eval_metrics")
        return {
            "status":  "error",
