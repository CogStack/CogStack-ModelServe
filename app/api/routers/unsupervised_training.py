...
from domain import ModelType, Tags
from exception import ClientException, ConfigurationException
...
async def train_unsupervised(
    ...
    training_data: Annotated[
        ...
        File(
            description=(
                "One or more files to be uploaded and each contains a list of plain texts, in the"
                ' format of ["text_1", "text_2", ..., "text_n"]'
            )
        ),
    ],
    epochs: Annotated[int, Query(description="The number of training epochs", ge=0)] = 1,
    lr_override: Annotated[
        Union[float, None], Query(description="The override of the initial learning rate", gt=0.0)
    ] = None,
    test_size: Annotated[
        Union[float, None], Query(description="The override of the test size in percentage", ge=0.0)
    ] = 0.2,
    ...
    description: Annotated[
        Union[str, None], Query(description="The description of the training or change logs")
    ] = None,
...
async def train_unsupervised_with_hf_dataset(
    request: Request,
    hf_dataset_repo_id: Annotated[
        Union[str, None],
        Query(
            description=(
                "The repository ID of the dataset to download from Hugging Face Hub, will be"
                " ignored when 'hf_dataset_package' is provided"
            )
        ),
    ] = None,
    hf_dataset_config: Annotated[
        Union[str, None],
        Query(
            description=(
                "The name of the dataset configuration, will be ignored when 'hf_dataset_package'"
                " is provided"
            )
        ),
    ] = None,
    hf_dataset_package: Annotated[
        Union[UploadFile, None],
        File(
            description=(
                "A ZIP file containing the dataset to be uploaded, will disable the download of"
                " 'hf_dataset_repo_id'"
            )
        ),
    ] = None,
    trust_remote_code: Annotated[
        bool, Query(description="Whether to trust the remote code of the dataset")
    ] = False,
    text_column_name: Annotated[
        str, Query(description="The name of the text column in the dataset")
    ] = "text",
    epochs: Annotated[int, Query(description="The number of training epochs", ge=0)] = 1,
    lr_override: Annotated[
        Union[float, None], Query(description="The override of the initial learning rate", gt=0.0)
    ...
    test_size: Annotated[
        ...
        Query(
            description=(
                "The override of the test size in percentage will only take effect if the dataset"
                " does not have predefined validation or test splits"
            ),
            ge=0.0,
        ),
    ] = 0.2,
    ...
    if hf_dataset_repo_id is None and hf_dataset_package is None:
        raise ClientException(
            "Either 'hf_dataset_repo_id' or 'hf_dataset_package' must be provided"
        )

    model_type = model_service.info().model_type
    if model_type not in [
        ModelType.HUGGINGFACE_NER,
        ModelType.MEDCAT_SNOMED,
        ModelType.MEDCAT_ICD10,
        ModelType.MEDCAT_UMLS,
    ]:
        raise ConfigurationException(
            f"Currently this endpoint is not available for models of type: {model_type.value}"
        )
    ...
    if hf_dataset_package is not None:
        ...
        for split in hf_dataset.keys():
            if text_column_name not in hf_dataset[split].column_names:
                raise ClientException(
                    f"The dataset does not contain a '{text_column_name}' column in the split(s)"
                )
            if text_column_name != "text":
                hf_dataset[split] = hf_dataset[split].map(
                    lambda x: {"text": x[text_column_name]}, batched=True
                )
            hf_dataset[split] = hf_dataset[split].remove_columns(
                [col for col in hf_dataset[split].column_names if col != "text"]
            )
