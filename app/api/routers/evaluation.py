...
async def get_evaluation_with_trainer_export(
    ...
    evaluation_accepted = model_service.train_supervised(
...
def get_sanity_check_with_trainer_export(
    ...
    try:
        ...
        for file in files:
            file.close()
    concatenated = filter_by_concept_ids(concatenated, model_service.info().model_type)
    metrics = sanity_check_model_with_trainer_export(
        concatenated, model_service, return_df=True, include_anchors=False
    )
    stream = io.StringIO()
    ...
    response.headers["Content-Disposition"] = (
        f'attachment ; filename="sanity_check_{tracking_id}.csv"'
    )
    return response
...
def get_inter_annotator_agreement_scores(
    request: Request,
    trainer_export: Annotated[
        List[UploadFile], File(description="A list of trainer export files to be uploaded")
    ],
    annotator_a_project_id: Annotated[int, Query(description="The project ID from one annotator")],
    annotator_b_project_id: Annotated[
        int, Query(description="The project ID from another annotator")
    ],
    scope: Annotated[
        str,
        Query(
            ...
            description=(
                "The scope for which the score will be calculated, e.g., per_concept, per_document"
                " or per_span"
            ),
        ),
    ],
    ...
    with tempfile.NamedTemporaryFile(mode="w+") as combined:
        ...
        if scope == Scope.PER_CONCEPT.value:
            iaa_scores = get_iaa_scores_per_concept(
                combined, annotator_a_project_id, annotator_b_project_id, return_df=True
            )
        elif scope == Scope.PER_DOCUMENT.value:
            iaa_scores = get_iaa_scores_per_doc(
                combined, annotator_a_project_id, annotator_b_project_id, return_df=True
            )
        elif scope == Scope.PER_SPAN.value:
            iaa_scores = get_iaa_scores_per_span(
                combined, annotator_a_project_id, annotator_b_project_id, return_df=True
            )
        else:
