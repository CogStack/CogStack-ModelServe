...
from domain import ModelCard, Tags, TextStreamItem, TextWithAnnotations, TextWithPublicKey
from utils import get_settings

import api.globals as cms_globals
from api.dependencies import validate_tracking_id
from api.utils import encrypt, get_rate_limiter
...
def get_entities_from_text(
    ...
    text: Annotated[
        str,
        Body(description="The plain text to be sent to the model for NER", media_type="text/plain"),
    ],
...
def get_entities_from_jsonlines_text(
    ...
    json_lines: Annotated[
        ...
        Body(
            description=(
                "The texts in the jsonlines format and each line contains"
                ' {"text": "<TEXT>"[, "name": "<NAME>"]}'
            ),
    ...
    try:
        ...
        return StreamingResponse(
            _get_jsonlines_stream(stream), media_type="application/x-ndjson; charset=utf-8"
        )
    except json.JSONDecodeError:
        return JSONResponse(
            status_code=HTTP_400_BAD_REQUEST, content={"message": "Invalid JSON Lines."}
        )
...
def get_entities_from_multiple_texts(
    ...
    texts: Annotated[
        ...
        Body(
            description=(
                "A list of plain texts to be sent to the model for NER, in the format of"
...
def extract_entities_from_multi_text_file(
    request: Request,
    multi_text_file: Annotated[
        UploadFile,
        File(
            description=(
                "A file containing a list of plain texts, in the format of"
                ' ["text_1", "text_2", ..., "text_n"]'
            )
        ),
    ],
    ...
    with tempfile.NamedTemporaryFile() as data_file:
        ...
        response.headers["Content-Disposition"] = (
            f'attachment ; filename="concatenated_{tracking_id}.json"'
        )
        return response
...
def get_redacted_text(
    ...
    concepts_to_keep: Annotated[
        List[str],
        Query(
            description=(
                "List of concepts (Label IDs) that should not be removedd during the redaction"
                " process. List should be in the format ['label1','label2'...]"
            )
        ),
    ] = [],
    warn_on_no_redaction: Annotated[
        Union[bool, None],
        Query(
            description=(
                "Return warning when no entities were detected for redaction to prevent potential"
                " info leaking"
            )
        ),
    ] = False,
    mask: Annotated[
        Union[str, None], Query(description="The custom symbols used for masking detected spans")
    ] = None,
    hash: Annotated[
        Union[bool, None], Query(description="Whether or not to hash detected spans")
    ] = False,
    model_service: AbstractModelService = Depends(cms_globals.model_service_dep),
) -> PlainTextResponse:
    annotations = model_service.annotate(text)
    ...
    if not annotations and warn_on_no_redaction:
        return PlainTextResponse(
            content="WARNING: No entities were detected for redaction.", status_code=200
        )
    else:
        for annotation in annotations:
            ...
            if hash:
                label = hashlib.sha256(
                    text[annotation["start"] : annotation["end"]].encode()
                ).hexdigest()
...
def get_redacted_text_with_encryption(
    ...
    if not annotations and warn_on_no_redaction:
        return JSONResponse(
            content={"message": "WARNING: No entities were detected for redaction."}
        )
    else:
        for idx, annotation in enumerate(annotations):
            label = f"[REDACTED_{idx}]"
            encrypted = encrypt(
                text_with_public_key.text[annotation["start"] : annotation["end"]],
                text_with_public_key.public_key_pem,
            )
    ...
    if annotations and annotations[0].get("accuracy", None) is not None:
        ...
        if config.LOG_PER_CONCEPT_ACCURACIES == "true":
            ...
            for concept, accumulated_accuracy in accumulated_concept_accuracy.items():
                ...
                cms_avg_anno_acc_per_concept.labels(handler=handler, concept=concept).set(
                    concept_avg_acc
                )
