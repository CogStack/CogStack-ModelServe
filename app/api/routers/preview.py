...
async def get_rendered_entities_from_text(
    request: Request,
    text: Annotated[
        str, Body(description="The text to be sent to the model for NER", media_type="text/plain")
    ],
...
def get_rendered_entities_from_trainer_export(
    ...
    trainer_export_str: Annotated[
        str, Form(description="The trainer export raw JSON string")
    ] = '{"projects": []}',
    ...
    document_id: Annotated[
        ...
        Query(
            description=(
                "The target document ID, and if not provided, all documents of the target"
                " project(s) will be included"
            )
        ),
    ] = None,
    tracking_id: Union[str, None] = Depends(validate_tracking_id),
    ...
    for project in data["projects"]:
        ...
        for document in project["documents"]:
            ...
            for annotation in document["annotations"]:
                correct_label = "correct" if annotation.get("correct", True) else "incorrect"
                is_terminated = annotation.get("deleted", False) and annotation.get("killed", False)
                terminated_label = "; terminated" if is_terminated else ""
                entities.append(
                    {
                        "start": annotation["start"],
                        "end": annotation["end"],
                        "label": f"{annotation['cui']} ({correct_label}{terminated_label})",
            ...
            logger.debug("Entities extracted for previewing %s", entities)
            doc = Doc(
                text=document["text"], ents=entities, title=f"P{project['id']}/D{document['id']}"
            )
            htmls.append(displacy.render(doc.dict(), style="ent", manual=True))
    if htmls:
        tracking_id = tracking_id or str(uuid.uuid4())
        response = StreamingResponse(
            BytesIO("<br/>".join(htmls).encode()), media_type="application/octet-stream"
        )
        response.headers["Content-Disposition"] = (
            f'attachment ; filename="preview_{tracking_id}.html"'
        )
    else:
        logger.debug("Cannot find any matching documents to preview")
        return JSONResponse(
            content={"message": "Cannot find any matching documents to preview"},
