...
async def train_supervised(
    ...
    test_size: Annotated[
        ...
        Query(
            description=(
                "The override of the test size in percentage. (For a 'huggingface-ner' model, a"
                " negative value can be used to apply the train-validation-test split if implicitly"
                " defined in trainer export: 'projects[0]' is used for training, 'projects[1]' for"
                " validation, and 'projects[2]' for testing)"
            )
        ),
    ] = 0.2,
    ...
    description: Annotated[
        Union[str, None], Form(description="The description of the training or change logs")
    ] = None,
    ...
    try:
        training_accepted = model_service.train_supervised(
            data_file,
            epochs,
            log_frequency,
            training_id,
