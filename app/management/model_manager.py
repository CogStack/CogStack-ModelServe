...
import shutil
import tempfile
from typing import Any, Dict, Iterator, List, Optional, Type, Union, final
...
class ModelManager(PythonModel):
    ...
    def __init__(self, model_service_type: Type, config: Settings) -> None:
        ...
        self._model_signature = ModelSignature(
            inputs=ModelManager.input_schema, outputs=ModelManager.output_schema, params=None
        )
    ...
    def retrieve_model_service_from_uri(
        mlflow_model_uri: str, config: Settings, downloaded_model_path: Optional[str] = None
        ...
        if downloaded_model_path:
            ModelManager.download_model_package(
                os.path.join(mlflow_model_uri, "artifacts"), downloaded_model_path
            )
    ...
    def download_model_package(model_artifact_uri: str, dst_file_path: str) -> Optional[str]:
        with tempfile.TemporaryDirectory() as dir_downloaded:
            mlflow.artifacts.download_artifacts(
                artifact_uri=model_artifact_uri, dst_path=dir_downloaded
            )
            ...
            if file_path:
                ...
                raise ManagedModelException(
                    "Cannot find the model .zip file inside artifacts downloaded from"
                    f" {model_artifact_uri}"
                )

    def log_model(
        self, model_name: str, model_path: str, registered_model_name: Optional[str] = None
    ) -> ModelInfo:
    ...
    def load_context(self, context: PythonModelContext) -> None:
        ...
        model_service = self._model_service_type(
            self._config,
            model_parent_dir=os.path.join(
                artifact_root, os.path.split(context.artifacts["model_path"])[0]
            ),
