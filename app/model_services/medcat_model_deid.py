...
from typing import Any, Callable, Dict, List, Optional, TextIO, final
...
from utils import get_hf_pipeline_device_id, non_default_device_is_available
...
class MedCATModelDeIdentification(MedCATModel):
    ...
    def annotate_with_local_chunking(self, text: str) -> Dict:
        ...
        for input_id, (start, end) in zip(input_ids, offset_mapping):
            ...
            if len(chunk) == MedCATModelDeIdentification.CHUNK_SIZE:
                ...
                c_text = text[
                    chunk[:last_token_start_idx][0][1][0] : chunk[:last_token_start_idx][-1][1][1]
                ]
                doc = self._with_lock(self.model.get_entities, c_text)
                doc["entities"] = {
                    _id: entity
                    for _id, entity in doc["entities"].items()
                    if (entity["end"] + processed_char_len) < chunk[window_overlap_start_idx][1][0]
                }
        ...
        total_char_len = len(text) + leading_ws_len
        assert (
            processed_char_len == total_char_len
        ), f"{total_char_len - processed_char_len} characters were not processed:\n{text}"
    ...
    def init_model(self) -> None:
        if hasattr(self, "_model") and isinstance(self._model, CAT):  # type: ignore
            ...
            self._model._addl_ner[0].tokenizer.hf_tokenizer._in_target_context_manager = getattr(
                self._model._addl_ner[0].tokenizer.hf_tokenizer, "_in_target_context_manager", False
            )
            self._model._addl_ner[0].tokenizer.hf_tokenizer.clean_up_tokenization_spaces = getattr(
                self._model._addl_ner[0].tokenizer.hf_tokenizer,
                "clean_up_tokenization_spaces",
                None,
            )
            self._model._addl_ner[0].tokenizer.hf_tokenizer.split_special_tokens = getattr(
                self._model._addl_ner[0].tokenizer.hf_tokenizer, "split_special_tokens", False
            )
            ...
            if "safe_serialization" in inspect.signature(_save_pretrained).parameters:
                self._model._addl_ner[0].model.save_pretrained = partial(
                    _save_pretrained,
    ...
    def _with_lock(self, func: Callable, *args: Any, **kwargs: Any) -> Any:
        # Temporarily tackle https://github.com/huggingface/tokenizers/issues/537
        # but it reduces parallelism
        with self._lock:
