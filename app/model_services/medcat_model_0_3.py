...
class MedCATModel(AbstractModelService):
    ...
    def __init__(self, config: Settings) -> None:
        ...
        self._model = self.load_model(self._model_pack_dir)
    ...
    def load_model(model_file_path: str, *args, **kwargs) -> CAT:
        ...
        cdb = CDB()
        cdb.load_dict(model_file_path + '/cdb.dat')

        vocab = Vocab()
        vocab.load_dict(model_file_path + '/vocab.dat')
...
class modelrunner:
    ...
    def __init__(self):
        cdb = CDB()
        cdb.load_dict(uclh_cdb)

        vocab = Vocab()
        vocab.load_dict(uclh_vocab)

        negated_model = MetaCAT(save_dir=uclh_negated_model_path)
        negated_model.load()
        self.model = CAT(cdb=cdb, vocab=vocab, meta_cats=[negated_model])
    ...
    def annotate(self, text):
        ...
        records = df[['label_name', 'label_id', 'start', 'end']].to_dict('records')
