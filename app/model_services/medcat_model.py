...
from utils import TYPE_ID_TO_NAME_PATCH, get_settings, non_default_device_is_available
...
class MedCATModel(AbstractModelService):
    def __init__(
        ...
        self._model_pack_path = os.path.join(
            self._model_parent_dir,
            config.BASE_MODEL_FILE if base_model_file is None else base_model_file,
        )
        self._enable_trainer = (
            enable_trainer if enable_trainer is not None else config.ENABLE_TRAINING_APIS == "true"
        )
        self._supervised_trainer = None
        self._unsupervised_trainer = None
        self._metacat_trainer = None
        self._whitelisted_tuis = set(
            [tui.strip() for tui in config.TYPE_UNIQUE_ID_WHITELIST.split(",")]
        )
    ...
    def _retrieve_meta_annotations(df: pd.DataFrame) -> pd.DataFrame:
        ...
        return pd.concat(
            [df.drop(["new_meta_anns"], axis=1), df["new_meta_anns"].apply(pd.Series)], axis=1
        )
    ...
    def annotate(self, text: str) -> Dict:
        doc = self.model.get_entities(
            text, addl_info=["cui2icd10", "cui2ontologies", "cui2snomed", "cui2athena_ids"]
        )
    ...
    def batch_annotate(self, texts: List[str]) -> List[Dict]:
        ...
        docs = self.model.multiprocessing(
        ...
        if df.empty:
            ...
            for idx, row in df.iterrows():
                if "athena_ids" in row and row["athena_ids"]:
                    df.loc[idx, "athena_ids"] = [
                        athena_id["code"] for athena_id in row["athena_ids"]
                    ]
    ...
    def _set_tuis_filtering(self) -> None:
        ...
        if self._whitelisted_tuis == {""}:
            return
        assert self._whitelisted_tuis.issubset(
            model_tuis
        ), f"Unrecognisable Type Unique Identifier(s): {self._whitelisted_tuis - model_tuis}"
