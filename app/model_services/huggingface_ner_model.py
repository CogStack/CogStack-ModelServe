...
from trainers.huggingface_ner_trainer import (
    HuggingFaceNerSupervisedTrainer,
    HuggingFaceNerUnsupervisedTrainer,
)
...
class HuggingFaceNerModel(AbstractModelService):
    ...
    def from_model(
        ...
        if non_default_device_is_available(get_settings().DEVICE):
            model_service._ner_pipeline = _pipeline(
                device=get_hf_pipeline_device_id(get_settings().DEVICE)
            )
        else:
            model_service._ner_pipeline = _pipeline()
    ...
    @staticmethod
    def load_model(
        model_file_path: str, *args: Tuple, **kwargs: Dict[str, Any]
    ) -> Tuple[PreTrainedModel, PreTrainedTokenizerBase]:
        model_path = os.path.join(
            os.path.dirname(model_file_path), os.path.basename(model_file_path).split(".")[0]
        )
    ...
    def batch_annotate(self, texts: List[str]) -> List[Dict]:
        raise NotImplementedError(
            "Batch annotation is not yet implemented for Hugging Face NER models"
        )
