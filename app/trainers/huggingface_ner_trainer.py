...
from typing import Any, Dict, Iterable, List, Optional, TextIO, Tuple, Union, final
...
from domain import DatasetSplit, Device, HfTransformerBackbone, ModelType
from exception import AnnotationException
from utils import filter_by_concept_ids, non_default_device_is_available, reset_random_seed
...
class HuggingFaceNerUnsupervisedTrainer(UnsupervisedTrainer):
    ...
    @staticmethod
    def run(
        trainer: "HuggingFaceNerUnsupervisedTrainer",
        ...
        try:
            ...
            test_size = (
                0.2 if training_params.get("test_size") is None else training_params["test_size"]
            )
            if isinstance(data_file, tempfile.TemporaryDirectory):
                ...
                if DatasetSplit.VALIDATION.value in raw_dataset.keys():
                    ...
                    train_texts = [
                        line.strip() for line in lines[: int(len(lines) * (1 - test_size))]
                    ]
                    eval_texts = [
                        line.strip() for line in lines[int(len(lines) * (1 - test_size)) :]
                    ]
            ...
            train_dataset = datasets.Dataset.from_generator(
                ...
                gen_kwargs={
                    "texts": train_texts,
                    "tokenizer": tokenizer,
                    "max_length": trainer._max_length,
                },
            ...
            training_args = TrainingArguments(
                ...
                save_total_limit=3,
                use_cpu=trainer._config.DEVICE.lower() == Device.CPU.value
                if non_default_device_is_available(trainer._config.DEVICE)
                else False,
            )
            ...
            if not skip_save_model:
                retrained_model_pack_path = os.path.join(
                    trainer._retrained_models_dir, f"{ModelType.HUGGINGFACE_NER.value}_{run_id}.zip"
                )
                ...
                shutil.make_archive(
                    retrained_model_pack_path.replace(".zip", ""), "zip", copied_model_directory
                )
                model_uri = trainer._tracker_client.save_model(
                    retrained_model_pack_path, trainer._model_name, trainer._model_manager
                )
    ...
    def _tokenize_and_chunk(
        texts: Iterable[str], tokenizer: PreTrainedTokenizerBase, max_length: int
        ...
        for text in texts:
            ...
            for i in range(0, len(encoded["input_ids"]), max_length):
                ...
                chunked_attention_mask = (
                    encoded["attention_mask"][i : i + max_length] + [0] * padding_length
                )
                chunked_special_tokens = tokenizer.get_special_tokens_mask(
                    chunked_input_ids, already_has_special_tokens=True
                )
...
class HuggingFaceNerSupervisedTrainer(SupervisedTrainer):
        ...
        def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
            return {
                "input_ids": torch.tensor(
                    [
                        self._add_padding(f["input_ids"], self.max_length, self.pad_token_id)
                        for f in features
                    ],
                    dtype=torch.long,
                ),
                "labels": torch.tensor(
                    [
                        self._add_padding(
                            f["labels"],
                            self.max_length,
                            HuggingFaceNerSupervisedTrainer.PAD_LABEL_ID,
                        )
                        for f in features
                    ],
                    dtype=torch.long,
                ...
                "attention_mask": torch.tensor(
                    [self._add_padding(f["attention_mask"], self.max_length, 0) for f in features],
                    dtype=torch.long,
                ),
            }
    ...
    @staticmethod
    def run(
        trainer: "HuggingFaceNerSupervisedTrainer",
        training_params: Dict,
        data_file: TextIO,
        log_frequency: int,
        run_id: str,
        ...
        if not eval_mode:
            try:
                ...
                filtered_training_data, filtered_concepts = (
                    trainer._filter_training_data_and_concepts(data_file)
                )
                ...
                test_size = (
                    0.2
                    if training_params.get("test_size") is None
                    else training_params["test_size"]
                )
                if test_size < 0:
                    logger.info(
                        "Using pre-defined train-validation-test split in trainer export..."
                    )
                    if len(filtered_training_data["projects"]) < 2:
                        raise AnnotationException(
                            "Not enough projects in the training data to provide a"
                    ...
                    documents = [
                        document
                        for project in filtered_training_data["projects"]
                    ...
                    train_documents = [
                        document for document in documents[: int(len(documents) * (1 - test_size))]
                    ]
                    eval_documents = [
                        document for document in documents[int(len(documents) * (1 - test_size)) :]
                    ]
                ...
                train_dataset.set_format(
                    type=None, columns=["input_ids", "labels", "attention_mask"]
                )
                eval_dataset.set_format(
                    type=None, columns=["input_ids", "labels", "attention_mask"]
                )

                data_collator = trainer._LocalDataCollator(
                    max_length=trainer._max_length, pad_token_id=tokenizer.pad_token_id
                )
                training_args = trainer._get_training_args(
                    results_path, logs_path, training_params, log_frequency
                )
                if training_params.get("lr_override") is not None:
                    training_args.learning_rate = training_params["lr_override"]
                ...
                hf_trainer = Trainer(
                    ...
                    compute_metrics=partial(
                        trainer._compute_token_level_metrics,
                        id2label=model.config.id2label,
                        tracker_client=trainer._tracker_client,
                        model_name=trainer._model_name,
                    ),
                    callbacks=[MLflowLoggingCallback(trainer._tracker_client)],
                )
                ...
                trainer._sanity_check_model_and_save_results(
                    data_file.name, trainer._model_service.from_model(model, tokenizer)
                )
            ...
            try:
                ...
                trainer._tracker_client.log_model_config(
                    trainer._model_service._model.config.to_dict()
                )
                trainer._tracker_client.log_trainer_version(transformers_version)
                with open(data_file.name, "r") as f:
                ...
                eval_documents = [
                    document
                    for project in eval_data["projects"]
                    for document in project["documents"]
                ]
                dataset_features = datasets.Features(
                    {
                ...
                eval_dataset = datasets.Dataset.from_generator(
                    ...
                    gen_kwargs={
                        "documents": eval_documents,
                        "tokenizer": trainer._model_service.tokenizer,
                        "max_length": trainer._max_length,
                        "model": trainer._model_service._model,
                    },
                    cache_dir=trainer._config.TRAINING_CACHE_DIR,
                )
                eval_dataset.set_format(
                ...
                data_collator = trainer._LocalDataCollator(
                    max_length=trainer._max_length,
                    pad_token_id=trainer._model_service.tokenizer.pad_token_id,
                )
                training_args = trainer._get_training_args(
                    results_path, logs_path, training_params, log_frequency
                )
                hf_trainer = Trainer(
                    ...
                    compute_metrics=partial(
                        trainer._compute_token_level_metrics,
                        id2label=trainer._model_service.model.config.id2label,
                        tracker_client=trainer._tracker_client,
                        model_name=trainer._model_name,
                    ),
                    tokenizer=None,
    ...
    def _filter_training_data_and_concepts(data_file: TextIO) -> Tuple[Dict, List]:
        with open(data_file.name, "r") as f:
            ...
            rear_concept_ids = te_stats_df[
                (te_stats_df["anno_count"] - te_stats_df["anno_ignorance_counts"])
                < HuggingFaceNerSupervisedTrainer.MIN_EXAMPLE_COUNT_FOR_TRAINABLE_CONCEPT
            ]["concept"].unique()
            logger.debug(
                "The following concept(s) will be excluded due to the low example count(s): %s",
                rear_concept_ids,
            )
            filtered_training_data = filter_by_concept_ids(
                training_data, ModelType.HUGGINGFACE_NER, extra_excluded=rear_concept_ids
            )
            filtered_concepts = get_stats_from_trainer_export(
                filtered_training_data, return_df=True
            )["concept"].unique()
    ...
    def _update_model_with_concepts(model: PreTrainedModel, concepts: List[str]) -> PreTrainedModel:
        if model.config.label2id == {"LABEL_0": 0, "LABEL_1": 1}:
            ...
            model.config.label2id = {
                "O": HuggingFaceNerSupervisedTrainer.DEFAULT_LABEL_ID,
                "X": HuggingFaceNerSupervisedTrainer.CONTINUING_TOKEN_LABEL_ID,
            }
            model.config.id2label = {
                HuggingFaceNerSupervisedTrainer.DEFAULT_LABEL_ID: "O",
                HuggingFaceNerSupervisedTrainer.CONTINUING_TOKEN_LABEL_ID: "X",
            }
        ...
        for concept in concepts:
            if concept not in model.config.label2id.keys():
                ...
                model.classifier.weight = nn.Parameter(
                    torch.cat((model.classifier.weight, avg_weight), 0)
                )
                model.classifier.bias = nn.Parameter(
                    torch.cat((model.classifier.bias, avg_bias), 0)
                )
    ...
    def _tokenize_and_chunk(
        ...
        for document in documents:
            ...
            document["annotations"] = sorted(
                document["annotations"], key=lambda annotation: annotation["start"]
            )
            for i in range(0, len(encoded["input_ids"]), max_length):
                chunked_input_ids = encoded["input_ids"][i : i + max_length]
                ...
                for annotation in document["annotations"]:
                    ...
                    label_id = model.config.label2id.get(
                        annotation["cui"], HuggingFaceNerSupervisedTrainer.DEFAULT_LABEL_ID
                    )
    ...
    def _compute_token_level_metrics(
        ...
        precision, recall, f1, support = precision_recall_fscore_support(
            non_padding_label_ids,
            non_padding_predictions,
            labels=list(id2label.keys()),
            average=None,
        )
        filtered_predictions, filtered_label_ids = zip(
            *[
                (a, b)
                for a, b in zip(non_padding_predictions, non_padding_label_ids)
                if not (a == b == HuggingFaceNerSupervisedTrainer.DEFAULT_LABEL_ID)
            ]
        )
        ...
        for idx, (label, precision, recall, f1, support) in enumerate(
            zip(
                labels[2 : HuggingFaceNerSupervisedTrainer.MAX_CONCEPTS_TO_TRACK + 2],
        ...
        HuggingFaceNerSupervisedTrainer._save_metrics_plot(
            aggregated_metrics, aggregated_labels, tracker_client, model_name
        )
    ...
    def _save_metrics_plot(
        metrics: List[Dict], concepts: List[str], tracker_client: TrackerClient, model_name: str
    ) -> None:
        try:
    ...
    def _get_training_args(
        self, results_path: str, logs_path: str, training_params: Dict, log_frequency: int
    ) -> TrainingArguments:
        return TrainingArguments(
            ...
            load_best_model_at_end=True,
            save_total_limit=3,
            use_cpu=self._config.DEVICE.lower() == Device.CPU.value
    ...
    def _sanity_check_model_and_save_results(
        self, data_file_path: str, model_service: AbstractModelService
    ) -> None:
        self._tracker_client.save_dataframe_as_csv(
            "sanity_check_result.csv",
            sanity_check_model_with_trainer_export(
                data_file_path, model_service, return_df=True, include_anchors=True
            ),
            self._model_name,
        )
